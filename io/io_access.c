...
* 修改日期        版本号     修改人       修改内容
* -----------------------------------------------
* 2013/01/23        V1.0      李文峰        创建
*
* 编译命令:arm-linux-gcc -o ioacc  io_access.c -I../include -L=./ -L/home/visint/src/arm/lib -lvispace -lsqlite3 -liconv
*
*Copyright (c) Visint Inc., 2002-2013. All Rights Reserved.
*******************************************************************************/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
/**************************************************************
【功能】      处理单元盘报文信息集,可以包含若干个子集，每个子集定长为36个字节子集头2个字节为信息集ID及索引，后32个字节为信息集的数据
【参数说明】
...
【提示】单元盘数据发送变化时调用此函数
***************************************************************/
static void handelUnitMessage(char chassis,char slot,char meaage[])
{
...
【功能】      告警处理
【参数说明】
            alarm_meaage     告警信息，可以包含若干条，告警信息为定长5个字节，分别为：告警号，信息集ID，索引号，信息项ID，信息项当前状态
            uchar_t      slot 槽位号（0-MAX_UNIT_COUNT）,0为背板（0-MAX_UNIT_COUNT）
【返回值说明】
【提示】
***************************************************************/
static void handelAlarm(char alarm_meaage[])
{
...
pwron_card_retry0:
                        ...
                        bi=getBaseInfoMap(0,i+1);
                         if (NULL==bi)
                           continue;
                        strncpy(bi->model,chassis_info.card_fix_info[i].Card_parm1,30);
                        strncpy(bi->creation,chassis_info.card_fix_info[i].Card_parm2,30);
         insertUnit(0,i+1,UC_OLP,1,*bi,0);
                    }
                }
