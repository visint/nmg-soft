...
pid_t findCCPid()
{
  return findPid(CCALL_NAME);
}
pid_t findModBusPid()
{
  return findPid(MOD_BUS_NAME);
}
...
int sendSigToModBus(int sig,int  sigval)
{
  return sendSig(findPid(MOD_BUS_NAME),sig,sigval);
}
int sendSigToCC(int sig,int  sigval)
{
  return sendSig(findPid(CCALL_NAME),sig,sigval);
}

int getMsgQ(msgbuf_t *rcvbuf,bool_t watting,uchar_t timeout)
{
    int infolen;
...
int sendMsgQ(msgbuf_t sndbuf,bool_t watting,uchar_t timeout)
{
 int msgflg=0;
 int msgqid;
 if (watting!=TRUE)
...
int sendMsgToCC(msgbuf_t msgbuf,bool_t watting,uchar_t timeout)
{
 // pid_t pid;
  int signum;
  union sigval sig_val;
